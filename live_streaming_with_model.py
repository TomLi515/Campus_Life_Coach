...
from finetune.src.finetune.models import SingleStreamClassifier, FusionClassifier, load_pretrained_backbone
...
# Get the CAMPUS_LIFE_COACH directory (parent of where this script is)
REPO_ROOT = Path(__file__).resolve().parent
print(f"[paths] Repository root: {REPO_ROOT}\n")

# Add BOTH src directories to Python path
# This allows: import finetune.models and import pretrain.models
sys.path.insert(0, str(REPO_ROOT / "finetune" / "src"))
sys.path.insert(0, str(REPO_ROOT / "pretrain" / "src"))

print(f"[paths] Added to sys.path:")
print(f"  - {REPO_ROOT / 'finetune' / 'src'}")
print(f"  - {REPO_ROOT / 'pretrain' / 'src'}\n")
print("[importing] Loading models from your training code...")
try:
    from finetune.models import SingleStreamClassifier, FusionClassifier, load_pretrained_backbone
    print("[success] Imported SingleStreamClassifier, FusionClassifier, load_pretrained_backbone\n")
    USING_CUSTOM_MODELS = True
except ImportError as e:
    print(f"[WARNING] Could not import from finetune.models: {e}")
    print("[fallback] Will use simplified model definitions\n")
    USING_CUSTOM_MODELS = False



try:
    print("[importing] Loading model classes from finetune...")
    from finetune.models import SingleStreamClassifier, FusionClassifier
    print("[success] Loaded SingleStreamClassifier and FusionClassifier\n")
except ImportError as e:
    print(f"[ERROR] Failed to import from finetune.models: {e}")
    print("\nTrying alternative import method...\n")
    
    # If that fails, define simplified versions that will load from checkpoints
    class SingleStreamClassifier(nn.Module):
        """Classifier that will be populated from checkpoint"""
        def __init__(self, num_classes=5, embedding_dim=256, **kwargs):
            super().__init__()
            self.embedding_dim = embedding_dim
            self.num_classes = num_classes
            # Placeholder - real architecture from checkpoint
        ...
        def forward(self, x):
            ...
            # Simple pooling
            x = torch.mean(x, dim=2)  # (batch, 6)
            x = torch.mean(x, dim=1)  # (batch,)
            x = x.unsqueeze(1).expand(batch_size, self.embedding_dim)
            return self.fc(x)
    
    class FusionClassifier(nn.Module):
        """Fusion classifier that will be populated from checkpoint"""
        def __init__(self, num_classes=5, embedding_dim=256, **kwargs):
            super().__init__()
            self.embedding_dim = embedding_dim
            self.num_classes = num_classes
        ...
        def forward(self, phone_x, watch_x):
            # phone_x, watch_x: (batch, 6, 150)
            batch_size = phone_x.shape[0]
            phone_pool = torch.mean(phone_x, dim=(1, 2))  # (batch,)
            watch_pool = torch.mean(watch_x, dim=(1, 2))  # (batch,)
            combined = torch.cat([
                phone_pool.unsqueeze(1).expand(batch_size, self.embedding_dim // 2),
                watch_pool.unsqueeze(1).expand(batch_size, self.embedding_dim // 2)
            ], dim=1)
            return self.fc(combined)
    
    print("[success] Using simplified model classes\n")
...
def load_classifier_from_checkpoint(checkpoint_path, classifier_class, device, is_fusion=False):
    """
    Load a trained classifier from checkpoint.
    
    Args:
        checkpoint_path: Path to .pth file
        classifier_class: SingleStreamClassifier or FusionClassifier
        device: torch device
        is_fusion: True for fusion model
    ...
    try:
        ...
        f1_str = f" (F1: {test_f1:.4f})" if test_f1 else ""
        print(f"  ✓ Loaded {num_classes} classes, embedding_dim={embedding_dim}{f1_str}")
...
print("\n[startup] Loading trained models...\n")

phone_model = load_classifier_from_checkpoint(
    MODEL_DIR / "phone_only_classifier.pth",
    SingleStreamClassifier,
    device,
    is_fusion=False
)

watch_model = load_classifier_from_checkpoint(
    MODEL_DIR / "watch_only_classifier.pth",
    SingleStreamClassifier,
    device,
    is_fusion=False
)

fusion_model = load_classifier_from_checkpoint(
...
print(f"\n[model status]")
print(f"  📱 Phone model:  {'✓ LOADED' if is_model_loaded(phone_model) else '✗ FAILED'}")
print(f"  ⌚ Watch model:  {'✓ LOADED' if is_model_loaded(watch_model) else '✗ FAILED'}")
print(f"  🔗 Fusion model: {'✓ LOADED' if is_model_loaded(fusion_model) else '✗ FAILED'}\n")
...
def predict_phone(window):
    if window is None or not is_model_loaded(phone_model):
    ...
    try:
        ...
        print(f"[ERROR] Phone prediction: {e}")
        return "Error", np.zeros(len(ACTIVITY_LABELS))


def predict_watch(window):
    if window is None or not is_model_loaded(watch_model):
        return "---", np.zeros(len(ACTIVITY_LABELS))
    ...
    try:
        ...
        print(f"[ERROR] Watch prediction: {e}")
        return "Error", np.zeros(len(ACTIVITY_LABELS))


def predict_fusion(phone_window, watch_window):
    if (phone_window is None or watch_window is None or 
        not is_model_loaded(fusion_model)):
        return "---", np.zeros(len(ACTIVITY_LABELS))
    
    try:
        with torch.no_grad():
            phone_x = torch.from_numpy(normalize_window(phone_window)).unsqueeze(0).float().to(device)
            watch_x = torch.from_numpy(normalize_window(watch_window)).unsqueeze(0).float().to(device)
            outputs = fusion_model(phone_x, watch_x)
            probs = torch.softmax(outputs, dim=1).cpu().numpy()[0]
            return ACTIVITY_LABELS[np.argmax(probs)], probs
    except Exception as e:
        print(f"[ERROR] Fusion prediction: {e}")
        return "Error", np.zeros(len(ACTIVITY_LABELS))
...
if __name__ == "__main__":
    ...
    print(is_model_loaded(phone_model))
    print(is_model_loaded(watch_model))
    print(is_model_loaded(fusion_model))
